...
Be strict. Only return valid JSON. No markdown, no code blocks, just JSON.`;
  ...
  async parsePrompt(prompt: string): Promise<ParsedPrompt> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `DeepSeek API error: ${response.status} - ${JSON.stringify(
            errorData
          )}`
        );
      }
  ...
  private validateAndConvert(
    ...
    if (amount < AMOUNT_BOUNDS.min || amount > AMOUNT_BOUNDS.max) {
      errors.push(
        `Amount out of bounds (${AMOUNT_BOUNDS.min} - ${AMOUNT_BOUNDS.max}): ${amount}`
      );
    }
    ...
    if (response.confidence < CONFIDENCE_THRESHOLD) {
      errors.push(
        `Confidence too low: ${response.confidence} (threshold: ${CONFIDENCE_THRESHOLD})`
      );
    }
    ...
    // Convert to TradeCommand (map action types)
    let action: 'swap' | 'market_buy' | 'market_sell';
    ...
    const command: TradeCommand = {
      action,
      baseToken:
        response.action === 'buy' ? response.toToken : response.fromToken,
      quoteToken:
        response.action === 'buy' ? response.fromToken : response.toToken,
      amount,
...
export function createDeepSeekClient(apiKey?: string): DeepSeekAI {
  ...
  if (!key) {
    throw new Error(
      'DeepSeek API key not found. Set VITE_DEEPSEEK_API_KEY environment variable.'
    );
  }
