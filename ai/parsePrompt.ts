...
export function parsePrompt(prompt: string): ParsedPrompt {
  ...
  if (lowerPrompt.includes('swap') || lowerPrompt.includes('convert')) {
    ...
    const swapMatch = prompt.match(/(?:swap|convert)\s+[\d.]+\s+(\w+)\s+(?:to|for|into)\s+(\w+)/i);
    if (swapMatch) {
    ...
    const buyMatch = prompt.match(/buy\s+[\d.]+\s+(\w+)\s+(?:with|using)\s+(\w+)/i);
    if (buyMatch) {
    ...
    const sellMatch = prompt.match(/sell\s+[\d.]+\s+(\w+)\s+(?:for|to)\s+(\w+)/i);
    if (sellMatch) {
