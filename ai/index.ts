...
export async function parseTradePrompt(
  ...
  if (fallbackToRegex) {
    ...
    if (result.confidence < 0.7) {
      throw new Error(
        `Unable to parse prompt with confidence. AI Error: ${aiError?.message || 'N/A'}. Regex confidence: ${result.confidence}`
      );
    }
  ...
  // Both failed
  throw new Error(
    `Failed to parse prompt. ${aiError ? `AI Error: ${aiError.message}` : 'AI disabled'}`
  );
}
...
export { createDeepSeekClient, getTokenIdentifier, isSupportedToken } from './deepseek';
